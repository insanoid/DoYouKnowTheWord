...
class DoYouKnowTheWordTests: XCTestCase {
    ...
    func testFiltering() throws {
        ...
        localisedContent.addContext(content: ["key_1": "CTX_1", "key_2": "CTX_2", "key_3": "CTX_3", "key_4": "CTX_4", "key_5": "CTX_5"])
        
        var result: [LocalisedItem]? = localisedContent.all("ValUe_1", filterType: .All)
        XCTAssertEqual(result!.count, 1)
        XCTAssertEqual(result![0].id, "key_1")
        
        result = localisedContent.all("ValUe_1", filterType: .Translated)
        XCTAssertEqual(result!.count, 1)
        XCTAssertEqual(result![0].id, "key_1")
        
        result = localisedContent.all("ValUe_1", filterType: .Untranslated)
        XCTAssertNil(result)
        
        result = localisedContent.all("ValUe_3_", filterType: .Untranslated)
        XCTAssertEqual(result!.count, 1)
        XCTAssertEqual(result![0].id, "key_3")
        
        result = localisedContent.all("CTX_4", filterType: .Untranslated)
        XCTAssertEqual(result!.count, 1)
        XCTAssertEqual(result![0].id, "CTX_4")
        
        result = localisedContent.all("key_3", filterType: .Untranslated)
        XCTAssertEqual(result!.count, 1)
        XCTAssertEqual(result![0].id, "key_3")
        
        result = localisedContent.all("key_3", filterType: .All)
        XCTAssertEqual(result!.count, 1)
        XCTAssertEqual(result![0].id, "key_3")
        
        result = localisedContent.all("CTX_4", filterType: .All)
        XCTAssertEqual(result!.count, 1)
        XCTAssertEqual(result![0].id, "CTX_4")
        
        result = localisedContent.all("CTX_", filterType: .All)
